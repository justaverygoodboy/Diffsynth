...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if model_resource == "civitai":
            ...
            state_dict_results = state_dict_converter.from_diffsynth(state_dict)
...
class ModelDetectorFromJointTraining:
    """专门用于检测联合训练后的模型"""
    def __init__(self):
        pass
    ...
    def match(self, file_path="", state_dict={}):
        ...
        # 检查是否包含修改后的DiT结构
        has_modified_dit = False
        for key in state_dict.keys():
            # 检查patch_embedding的通道数
            if "patch_embedding.weight" in key:
                weight_shape = state_dict[key].shape
                if len(weight_shape) >= 2:
                    in_channels = weight_shape[1]  # Conv3d的输入通道维度
                    if in_channels in [32, 52]:  # 修改后的通道数
                        has_modified_dit = True
                        break
            # 检查head的输出维度
                ...
                if len(weight_shape) >= 1:
                    ...
                    # 检查是否是扩展后的输出维度
                    if out_features % 32 == 0:  # RGB(16) + Depth(16) 相关
                        has_modified_dit = True
                        break
        
        return has_modified_dit
    ...
    def load(self, file_path="", state_dict={}, device="cuda", torch_dtype=torch.float16, **kwargs):
        ...
        try:
            ...
            print(f"    Failed to load joint training model: {e}")
            return [], []
...
class ModelManager:
    def __init__(
        ...
        self.model_detector = [
            # ModelDetectorFromJointTraining(),
